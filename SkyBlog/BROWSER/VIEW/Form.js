		...
		inner.on('paramsChange', function(params) {
			...
			function(next) {
				...
				if (id === undefined) {
					...
					SkyBlog.ArticleModel.get(id, next);
				}
			},
							...
							c : [categoryInput = UUI.FULL_INPUT({
								...
								success : function(fileData, form) {
									...
									if (fileData.type.substring(0, 6) === 'image/') {
										editor.setValue(editor.getValue() + '[![ScreenShot](' + SkyBlog.RF('THUMB/' + fileData.id) + ')](' + SkyBlog.RF(fileData.id) + ')', -1);
									} else {
										editor.setValue(editor.getValue() + '[](' + SkyBlog.RF(fileData.id) + ')', -1);
									}
								}
							})],
							on : {
								submit : function(e, form) {
									...
									(articleData === undefined ? SkyBlog.ArticleModel.create : SkyBlog.ArticleModel.update)(data, {
