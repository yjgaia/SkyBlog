SkyBlog.ArticleDom = CLASS(function(cls) {
	'use strict';
	...
	return {
		...
		init : function(inner, self, params) {
			...
			panel = UUI.PANEL({
				...
				c : [H3({
					...
					c : [(isViewMode === true ? SPAN : A)({
						...
						href : isViewMode === true ? undefined : SkyBlog.HREF('view/' + articleData.id),
					...
					on : {
						tap : function(e) {
							if (isViewMode !== true) {
								SkyBlog.GO('view/' + articleData.id);
							}
						}
					}
				}), DIV({
					...
					c : [SkyBlog.Layout.checkIsAuthed() === true ? DIV({
						style : {
							flt : 'right',
							color : '#4183c4',
							marginBottom : 10
						...
						c : [A({
							...
							on : {
								tap : function() {
									SkyBlog.GO('update/' + articleData.id);
								}
							}
						}), ' ', A({
							c : '글 삭제',
							on : {
								tap : function() {
									...
									if (confirm('정말 삭제하시겠습니까?') === true) {
										SkyBlog.ArticleModel.remove(articleData.id, function() {
											SkyBlog.REFRESH('');
										});
									}
								}
					...
					}), BROWSER_CONFIG.SkyBlog.isUsingComment === true ? UUI.VALID_FORM({
						errorMsgs : {
							name : {
						...
						on : {
							submit : function(e, form) {
								...
								SkyBlog.CommentModel.create(data, {
			...
			onNewCommentRoom = SkyBlog.CommentModel.onNewAndFindWatching({
				properties : {
